        ...
        def do_GET(self):
            ...
            try:
                ...
                if url.path == '/api/invoices':
                    ...
                    customer = params.get('customer', ['all'])[0]
                    return self.send(200, reporting.invoices(db, status=status, customer=customer))
                if url.path == '/api/export':
                    params = parse_qs(url.query)
                    status = params.get('status', ['all'])[0]
                    customer = params.get('customer', ['all'])[0]
                    return self.send(200, reporting.export_csv(db, status=status, customer=customer), 'text/csv; charset=utf-8')
